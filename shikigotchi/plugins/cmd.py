...
from shikigotchi.utils import download_file, unzip, save_config, parse_version, md5
from shikigotchi.plugins import default_path


SAVE_DIR = '/usr/local/share/shikigotchi/available-plugins/'
DEFAULT_INSTALL_PATH = '/usr/local/share/shikigotchi/installed-plugins/'
...
def add_parsers(subparsers):
    ...
    # shikigotchi plugins search
    parser_plugins_search = plugin_subparsers.add_parser('search', help='Search for shikigotchi plugins')
    parser_plugins_search.add_argument('pattern', type=str, help="Search expression (wildcards allowed)")

    # shikigotchi plugins list
    parser_plugins_list = plugin_subparsers.add_parser('list', help='List available shikigotchi plugins')
    parser_plugins_list.add_argument('-i', '--installed', action='store_true', required=False, help='List also installed plugins')

    # shikigotchi plugins update
    parser_plugins_update = plugin_subparsers.add_parser('update', help='Updates the database')

    # shikigotchi plugins upgrade
...
def edit(args, config):
    ...
    from shikigotchi.utils import DottedTomlEncoder
...
def list_plugins(args, config, pattern='*'):
    ...
    if not max_len_list:
        print('Maybe try: sudo shikigotchi plugins update')
        return 1
