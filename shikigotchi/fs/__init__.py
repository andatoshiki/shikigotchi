...
def setup_mounts(config):
    ...
    for name, options in fs_cfg['mounts'].items():
        ...
        target = os.path.join('/run/shikigotchi/disk/', os.path.basename(options['mount']))
...
class MemoryFS:
    ...
    def mount(self):
        ...
        if self.zram and self.zdev is not None:
            if os.system(f"mount -t {self.zram_fs_type} -o nosuid,noexec,nodev,user=shikigotchi /dev/zram{self.zdev} {self.mountpoint}/"):
                return False
        else:
            if os.system(f"mount -t tmpfs -o nosuid,noexec,nodev,mode=0755,size={self.size} shikigotchi {self.mountpoint}/"):
                return False
