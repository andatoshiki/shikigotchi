...
from shikigotchi import reboot
...
def run_task(name, options):
    ...
    with open('/etc/systemd/system/%s' % task_service_name, 'wt') as task_service:
        task_service.write("""
        [Unit]
        Description=Executes the tasks of the shikigotchi switcher plugin
        After=shikigotchi.service bettercap.service
    ...
    if 'reboot' in options and options['reboot']:
        ...
        # add condition
        systemd_dropin("shikigotchi.service", """
        [Unit]
