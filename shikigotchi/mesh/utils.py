...
import shikigotchi.ui.faces as faces
import shikigotchi.plugins as plugins
import shikigotchi.grid as grid
from shikigotchi.mesh.peer import Peer
...
class AsyncAdvertiser(object):
    def __init__(self, config, view, keypair):
        ...
        self._advertisement = {
            ...
            'version': shikigotchi.__version__,
    ...
    def _update_advertisement(self, s):
        self._advertisement['shikid_run'] = len(self._handshakes)
        self._advertisement['shikid_tot'] = utils.total_unique_handshakes(self._config['bettercap']['handshakes'])
        self._advertisement['uptime'] = shikigotchi.uptime()
    ...
    def _adv_poller(self):
        ...
        while True:
            try:
                logging.debug("polling shikigrid-peer for peers ...")
                ...
                logging.warning("error while polling shikigrid-peer: %s" % e)
