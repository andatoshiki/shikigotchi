...
from shikigotchi.ui.components import Text
...
class FixServices(plugins.Plugin):
    ...
    def on_epoch(self, agent, epoch, epoch_data):
        ...
        other_other_last_lines = ''.join(
            list(TextIOWrapper(subprocess.Popen(['tail', '-n10', '/var/log/shikigotchi.log'],
                                                stdout=subprocess.PIPE).stdout))[-10:])
        ...
        if time.time() - self.LASTTRY > 180:
            ...
            # Look for pattern 5
                ...
                if hasattr(agent, 'view'):
                    display = agent.view()
                    display.set('status', 'Restarting shikigotchi!')
                    display.update(force=True)
...
if __name__ == "__main__":
    ...
    agent = Client('localhost', port=8081, username="shikigotchi", password="shikigotchi")
