...
def load_config(args):
    ...
    import shikigotchi
    ref_defaults_file = os.path.join(os.path.dirname(shikigotchi.__file__), 'defaults.toml')
    ...
    for boot_conf in ['/boot/config.yml', '/boot/firmware/config.yml', '/boot/config.toml', '/boot/firmware/config.toml']:
        if os.path.exists(boot_conf):
            if os.path.exists(args.user_config):
                # if /etc/shikigotchi/config.toml already exists we just merge the new config
    ...
    # check for an entire shikigotchi folder on /boot/
    if os.path.isdir('/boot/firmware/shikigotchi'):
        print("installing /boot/firmware/shikigotchi to /etc/shikigotchi ...")
        shutil.rmtree('/etc/shikigotchi', ignore_errors=True)
        shutil.move('/boot/firmware/shikigotchi', '/etc/')
...
def extract_from_pcap(path, fields):
    ...
    for field in fields:
        ...
        if field == WifiInfo.BSSID:
            ...
            from shikigotchi.mesh.wifi import freq_to_channel
...
class StatusFile(object):
    ...
    def update(self, data=None):
        from shikigotchi.fs import ensure_write
