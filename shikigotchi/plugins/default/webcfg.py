...
import _thread
import shikigotchi
from shikigotchi import restart, plugins
from shikigotchi.utils import save_config, merge_config
...
class WebConfig(plugins.Plugin):
    ...
    def on_webhook(self, path, request):
        ...
        if request.method == "GET":
            ...
            if path == "save-config":
                try:
                    save_config(request.get_json(), '/etc/shikigotchi/config.toml')  # test
                ...
                try:
                    ...
                    shikigotchi.config = merge_config(request.get_json(), shikigotchi.config)
                    logging.debug("SHIKIGOTCHI CONFIG:\n%s" % repr(shikigotchi.config))
                    if self._agent:
