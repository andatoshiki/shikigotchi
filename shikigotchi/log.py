...
LAST_SESSION_FILE = '/root/.shikigotchi-last-session'
...
class LastSession(object):
    ...
    def __init__(self, config):
        ...
        self._peer_parser = re.compile(
            'detected unit (.+)@(.+) \(v.+\) on channel \d+ \(([\d\-]+) dBm\) \[sid:(.+) shikid_tot:(\d+) uptime:(\d+)]')
        self.parsed = False
        ...
        for line in self.last_session:
            ...
            try:
                ...
                if LastSession.DEAUTH_TOKEN in line and line not in cache:
                    ...
                    if m:
                        name, pubkey, rssi, sid, shikid_tot, uptime = m[0]
                        if pubkey not in cache:
                            self.last_peer = Peer({
                                ...
                                'advertisement': {
                                    'name': name,
                                    'shikid_tot': int(shikid_tot)
                                }})
                            self.peers += 1
                            cache[pubkey] = self.last_peer
                        else:
                            cache[pubkey].adv['shikid_tot'] = shikid_tot
