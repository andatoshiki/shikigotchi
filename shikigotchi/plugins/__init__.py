...
def toggle_plugin(name, enable=True):
    ...
    """
    import shikigotchi
    from shikigotchi.ui import view
    from shikigotchi.utils import save_config

    global loaded, database

    if shikigotchi.config:
        if name not in shikigotchi.config['main']['plugins']:
            shikigotchi.config['main']['plugins'][name] = dict()
        shikigotchi.config['main']['plugins'][name]['enabled'] = enable
        save_config(shikigotchi.config, '/etc/shikigotchi/config.toml')
    ...
    if enable and name in database and name not in loaded:
        ...
        if name in loaded and shikigotchi.config and name in shikigotchi.config['main']['plugins']:
            loaded[name].options = shikigotchi.config['main']['plugins'][name]
        one(name, 'loaded')
        if shikigotchi.config:
            one(name, 'config_changed', shikigotchi.config)
