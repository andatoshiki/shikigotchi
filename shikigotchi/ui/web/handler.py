...
import shikigotchi
import shikigotchi.grid as grid
import shikigotchi.ui.web as web
...
class Handler:
    ...
    def inbox(self):
        ...
        try:
            ...
            logging.exception('error while reading shikimail inbox')
            error = str(e)

        return render_template('inbox.html',
                               name=shikigotchi.name(),
                               page=page,
                               error=error,
                               inbox=inbox)
    ...
    def inbox_profile(self):
        ...
        try:
            ...
            logging.exception('error while reading shikigrid data')
            error = str(e)
    ...
    def inbox_peers(self):
        ...
        try:
            ...
            logging.exception('error while reading shikigrid peers')
            error = str(e)
    ...
    def show_message(self, id):
        ...
        try:
            ...
            logging.exception('error while reading shikimail message %d' % int(id))
            error = str(e)
    ...
    def plugins(self, name, subpath):
        ...
        if name == 'upgrade' and request.method == 'POST':
            ...
            os.system(f"shikigotchi plugins update && shikigotchi plugins upgrade {request.form['plugin']}")
    ...
    def shutdown(self):
        try:
            return render_template('status.html', title=shikigotchi.name(), go_back_after=60,
                                   message='Shutting down ...')
        finally:
            _thread.start_new_thread(shikigotchi.shutdown, ())
    ...
    def reboot(self):
          try:
              ...
              _thread.start_new_thread(shikigotchi.reboot, ())
    ...
    def restart(self):
        ...
        try:
            return render_template('status.html', title=shikigotchi.name(), go_back_after=30,
                                   message='Restarting in %s mode ...' % mode)
        finally:
            _thread.start_new_thread(shikigotchi.restart, (mode,))
