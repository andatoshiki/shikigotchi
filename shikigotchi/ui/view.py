...
from shikigotchi.ui.components import *
from shikigotchi.ui.state import State
from shikigotchi.voice import Voice
...
class View(object):
    def __init__(self, config, impl, state=None):
        ...
        self._state = State(state={
            ...
            'name': Text(value='%s>' % 'shikigotchi', position=self._layout['name'], color=BLACK, font=fonts.Bold),
    ...
    def set_closest_peer(self, peer, num_total):
        if peer is None:
            ...
            name = '▌' * num_bars
            name += '│' * (4 - num_bars)
            name += ' %s %d (%d)' % (peer.name(), peer.shikid_run(), peer.shikid_total())
