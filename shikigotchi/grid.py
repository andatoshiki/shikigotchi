...
# shikigrid-peer is running on port 8666
...
def is_connected():
    try:
        # check DNS
        host = 'https://grid-api.toshiki.dev/api/v1/uptime'
...
def update_data(last_session):
    ...
    try:
        ...
        pass
    enabled = [name for name, options in shikigotchi.config['main']['plugins'].items() if
               'enabled' in options and options['enabled']]
    language = shikigotchi.config['main']['lang']
    ai = shikigotchi.config['ai']['enabled']
    ...
    data = {
        ...
        'build': "Shikigotchi by Anda Toshiki @andatoshiki",
        'plugins': enabled,
        'language': language,
        'bettercap': subprocess.getoutput("bettercap -version").split(".\n\n")[1],
        'shikigrid': subprocess.getoutput("shikigrid -version")
    }
