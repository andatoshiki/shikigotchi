...
from shikigotchi.utils import StatusFile, parse_version as version_to_tuple
...
def check(version, repo, native=True):
    ...
    resp = requests.get("https://gh-api.toshiki.dev/repos/%s/releases/latest" % repo)
    latest = resp.json()
        ...
        with self.lock:
            ...
            try:
                ...
                to_check = [
                    ...
                    ('andatoshiki/shikigrid', parse_version('shikigrid -version'), True, 'shikigrid-peer'),
                    ('andatoshiki/shikigotchi', shikigotchi.__version__, False, 'shikigotchi')
                ]
