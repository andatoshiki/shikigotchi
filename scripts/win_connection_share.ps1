<#
    .SYNOPSIS
        A script that setups Internet Connection Sharing for Shikigotchi.
    ...
    .PARAMETER SetShikigotchiSubnet
        Change the Internet Connection Sharing subnet to the Shikigotchi subnet. The USB Gadget Interface IP will default to 10.0.0.1.
    ...
    .EXAMPLE
        # Change the regkeys of Internet Connection Sharing to the Shikigotchi Subnet
        PS C:\> .\win_connection_share -SetShikigotchiSubnet

    .EXAMPLE
        # Change the regkeys of Internet Connection Sharing to the Shikigotchi Subnet with a custom ScopeAddress (The IP Address of the USB Gadget Interface.)
        PS C:\> .\win_connection_share -SetShikigotchiSubnet -ScopeAddress 10.0.0.10
#>
...
Function Test-ShikigotchiSubnet {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
        PS> Test-ShikigotchiSubnet
        [!] By default Internet Connection Sharing uses a 192.168.137.x subnet. Run Set-ShikigotchiSubnet to ensure you and your little friend are on the same subnet.
    #>
    [Cmdletbinding()]
    ...
    If ($RegKeys.ScopeAddress -notmatch '10.0.0.') {
        Write-Error "By default Internet Connection Sharing uses a 192.168.137.x subnet. Run Set-ShikigotchiSubnet to ensure you and your little friend are on the same subnet." -ErrorAction Stop
    }
    If ($RegKeys.ScopeAddressBackup -notmatch '10.0.0.') {
        Write-Error "By default Internet Connection Sharing uses a 192.168.137.x subnet. Run Set-ShikigotchiSubnet to ensure you and your little friend are on the same subnet." -ErrorAction Stop
    } 
}
Function Set-ShikigotchiSubnet {
    <#
    .SYNOPSIS
...
# Main Function
Function Setup-ShikigotchiNetwork {
    <#
    .SYNOPSIS
    ...
    .PARAMETER SetShikigotchiSubnet
        Change the Internet Connection Sharing subnet to the Shikigotchi. Defaults to 10.0.0.1.
    
    .PARAMETER ScopeAddress
        Custom ScopeAddress (the ICS ip address)
    
    .EXAMPLE
        PS> Setup-ShikigotchiNetwork -EnableInternetConnectionSharing
    ...
    Begin {
        Try {
            ...
            Try {
                Test-ShikigotchiSubnet @ErrorSplat
            } Catch {
                If ($SetShikigotchiSubnet) {
                    Write-Debug "Setting subnet"
                    Set-ShikigotchiSubnet -ScopeAddress $ScopeAddress @ErrorSplat
                } Else {
                    Write-Error "By default Internet Connection Sharing uses a 192.168.137.x subnet. Run this script with the -SetShikigotchiSubnet to setup the network." -ErrorAction Stop
                }
            }
        } Catch {
            $PSCmdlet.ThrowTerminatingError($PSItem)
...
# Dynamically create params for Setup-ShikigotchiNetwork function based of param input of script.
Setup-ShikigotchiNetwork @psBoundParameters
