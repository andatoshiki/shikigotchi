...
import shikigotchi
import shikigotchi.utils as utils
import shikigotchi.plugins as plugins
from shikigotchi.ui.web.server import Server
from shikigotchi.automata import Automata
from shikigotchi.log import LastSession
from shikigotchi.bettercap import Client
from shikigotchi.mesh.utils import AsyncAdvertiser
from shikigotchi.ai.train import AsyncTrainer

RECOVERY_DATA_FILE = '/root/.shikigotchi-recovery'
...
class Agent(Client, Automata, AsyncAdvertiser, AsyncTrainer):
    def __init__(self, view, config, keypair):
        Client.__init__(self,
                        ...
                        "shikigotchi" if "username" not in config['bettercap'] else config['bettercap']['username'],
                        "shikigotchi" if "password" not in config['bettercap'] else config['bettercap']['password'])
        ...
        logging.info("%s@%s (v%s)", shikigotchi.name(), self.fingerprint(), shikigotchi.__version__)
    ...
    def _update_handshakes(self, new_shakes=0):
        ...
        if self._last_shikid is not None:
            txt += ' [%s]' % self._last_shikid
    ...
    def _load_recovery_data(self, delete=True, no_exceptions=True):
        try:
            with open(RECOVERY_DATA_FILE, 'rt') as fp:
                ...
                self._last_shikid = data['last_shikid']
        ...
        if jmsg['tag'] == 'wifi.client.handshake':
            ...
            if key not in self._handshakes:
                ...
                if ap_and_station is None:
                    ...
                    self._last_shikid = ap_mac
                    plugins.on('handshake', self, filename, ap_mac, sta_mac)
                else:
                    (ap, sta) = ap_and_station
                    self._last_shikid = ap['hostname'] if ap['hostname'] != '' and ap[
